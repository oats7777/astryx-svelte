 ...
 * @input Svelte package CSS entrypoints
...
const tailwindStylesPath = resolve(process.cwd(), 'src/lib/tailwind.css');
...
describe('styles.css token contract', () => {
  ...
  it('Given plain CSS consumers When the base entrypoint loads Then Tailwind directives stay isolated', async () => {
    const css = await readFile(stylesPath, 'utf8');
    const tailwindCss = await readFile(tailwindStylesPath, 'utf8');

    expect(css).toContain('@import "@astryxdesign/tokens/default.css"');
    expect(css).not.toContain('@import "@astryxdesign/tokens/tailwind-theme.css"');
    expect(css).not.toContain('@source ');
    expect(tailwindCss).toContain('@import "./styles.css";');
    expect(tailwindCss).toContain('@import "@astryxdesign/tokens/tailwind-theme.css";');
    expect(tailwindCss).toContain('@source "./**/*.{svelte,ts,js}";');
  });
