...
{#if shouldRender}
  <nav class="astryx-pagination" aria-label={label} data-testid={testId}>
    <button type="button" class="astryx-pagination__button" aria-label="Go to previous page" disabled={!canPrevious} onclick={() => go(page - 1)}>
      Previous
    </button>
    {#if variant === 'count' && totalItems != null}
      <span class="astryx-pagination__status">{(page - 1) * safePageSize + 1}–{Math.min(page * safePageSize, totalItems)} of {totalItems}</span>
    {:else if variant === 'compact' && computedTotalPages != null}
      <span class="astryx-pagination__status">Page {page} of {computedTotalPages}</span>
    {:else if variant === 'dots' && computedTotalPages != null}
      <span class="astryx-pagination__indicators" role="group" aria-label="Page indicators">
        {#each Array.from({length: computedTotalPages}, (_value, index) => index + 1) as item}
          <button type="button" class="astryx-pagination__button" aria-label={`Go to page ${item}`} aria-current={item === page ? 'page' : undefined} disabled={disabled} onclick={() => go(item)}>
            {item}
          </button>
        {/each}
      </span>
    {:else if variant === 'pages' && computedTotalPages != null}
      {#each pageRange as item}
        {#if item === '...'}
          <span class="astryx-pagination__ellipsis" data-ellipsis="true" aria-hidden="true">...</span>
        {:else}
          <button type="button" class="astryx-pagination__button" aria-label={`Go to page ${item}`} aria-current={item === page ? 'page' : undefined} disabled={disabled} onclick={() => go(item)}>
            {item}
          </button>
      ...
      {/each}
    {/if}
    <button type="button" class="astryx-pagination__button" aria-label="Go to next page" disabled={!canNext} onclick={() => go(page + 1)}>
      Next
    </button>
  </nav>
{/if}
