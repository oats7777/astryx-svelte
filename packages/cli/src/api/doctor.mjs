...
function detectStylesEntrypoint(cwd) {
  const styleEntrypoints = [
    {kind: 'plain', importPath: '@astryxdesign/svelte/styles.css'},
    {kind: 'tailwind', importPath: '@astryxdesign/svelte/tailwind.css'},
  ];
  const candidates = [
    'src/main.ts',
    'src/main.js',
  ...
  for (const file of candidates) {
    ...
    const entrypoint = styleEntrypoints.find(entry => source.includes(entry.importPath));
    if (entrypoint) return entrypoint.kind;
  }
  return null;
}

function hasStylesImport(cwd) {
  return detectStylesEntrypoint(cwd) !== null;
}
...
function checkStylesImport(ctx) {
  ...
  return {
    ...
    status: ok ? 'pass' : 'warn',
    message: ok
      ? 'An Astryx Svelte CSS entrypoint is imported by the app.'
      : 'Could not find an @astryxdesign/svelte/styles.css or /tailwind.css import in common app entry files.',
    fix: ok
      ? undefined
      : 'Import `@astryxdesign/svelte/styles.css` once for plain CSS, or `@astryxdesign/svelte/tailwind.css` for Tailwind.',
  };
}
...
function checkTailwind(ctx) {
  ...
  const expectsTailwind = ctx.stylesEntrypoint === 'tailwind';
  return {
    id: 'tailwind',
    label: 'Tailwind availability',
    status: hasTailwind ? 'pass' : expectsTailwind ? 'warn' : 'info',
    message: hasTailwind
      ? 'Tailwind is declared for token-backed utilities.'
      : expectsTailwind
        ? 'The Tailwind entrypoint is imported, but Tailwind is not declared.'
        : 'Tailwind is not declared; plain CSS usage is supported.',
    fix: hasTailwind || !expectsTailwind
      ? undefined
      : 'Install Tailwind or switch to `@astryxdesign/svelte/styles.css` for plain CSS.',
  };
}
...
export async function runChecks(options = {}) {
  ...
  const ctx = {
    ...
    stylesEntrypoint: detectStylesEntrypoint(cwd),
  };
