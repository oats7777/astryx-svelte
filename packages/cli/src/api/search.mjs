 ...
 * component's .doc.mjs.
 * @param {string} svelteDir
 */
async function gatherComponents(svelteDir) {
  const grouped = discoverSvelteComponents(svelteDir);
  const names = Object.values(grouped).flat();
  const candidates = [];
  for (const comp of names) {
    const readme = findSvelteComponentDoc(svelteDir, comp);
    let keywords = [];
    let description = '';
    ...
    candidates.push({
      ...
      keywords,
      description,
      _import: resolveSvelteImportPath(svelteDir, comp),
    });
  }
  return candidates;
}
...
export async function search(query, options = {}) {
  ...
  if (!svelteDir) {
    throw new AstryxError('Could not find @astryxdesign/svelte package');
  }
  ...
  const [components, hooks, docTopics, templates] = await Promise.all([
    wants('component') ? gatherComponents(svelteDir) : [],
    wants('hook') ? gatherHooks() : [],
