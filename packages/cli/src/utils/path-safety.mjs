...
function canonicalizeThroughExistingAncestors(absPath, label) {
  ...
  try {
    ...
    throw new PathSafetyError(
      `Invalid ${label} "${absPath}": contains an unresolved symlink.`,
      'PATH_TRAVERSAL',
    );
  }

  return missingSegments.length > 0
    ? path.join(realCursor, ...missingSegments)
    : realCursor;
}

/**
 * Resolve `targetPath` (relative to `rootDir`) and assert that the
 * result stays inside `rootDir`, including after resolving existing
 * symlink ancestors. Rejects:
 *
 ...
 *   - Paths whose existing symlink ancestors resolve outside `rootDir`.
 *
...
export function assertWithin(targetPath, rootDir, options = {}) {
  ...
  const realRoot = canonicalizeThroughExistingAncestors(absRoot, label);
  const realResolved = canonicalizeThroughExistingAncestors(resolved, label);

  if (!isWithinPath(realResolved, realRoot)) {
    throw new PathSafetyError(
      `Invalid ${label} "${targetPath}": resolves outside the project root ` +
        `(${realRoot}). Path traversal is not allowed.`,
      'PATH_TRAVERSAL',
    );
  }
 ...
 *   `astryx template blank ./foo.svelte`
 * writes to `./foo.svelte` as a file, not `./foo.svelte/page.svelte`.
 *
