...
/**
 * @file Check for newer @astryxdesign/svelte versions via local signals.
 *
 * Resolution order:
...
/**
 * Get the currently installed @astryxdesign/svelte version from the consumer's package.json.
 *
...
export function getInstalledVersion(cwd = process.cwd()) {
  try {
    ...
    const version = deps['@astryxdesign/svelte'];
...
export function checkForUpdate(cwd = process.cwd()) {
  ...
  if (semverGt(latest, installed)) {
    return `FYI: A newer version of @astryxdesign/svelte (${latest}) is available. Install the new package version, then run: astryx upgrade --from <old-version> --apply`;
  }
