...
describe('assertWithin', () => {
  ...
  it('rejects a symlinked output directory that resolves outside root', () => {
    const root = path.join(tmpDir, 'project');
    const escaped = path.join(tmpDir, 'escaped');
    fs.mkdirSync(root);
    fs.mkdirSync(escaped);
    fs.symlinkSync('../escaped', path.join(root, 'link-out'));

    expect(() => assertWithin('link-out', root)).toThrow(PathSafetyError);
    expect(() => assertWithin('link-out', root)).toThrow(/symlink|outside|traversal/i);
  });

  it('rejects a child path under a symlinked output directory outside root', () => {
    const root = path.join(tmpDir, 'project');
    const escaped = path.join(tmpDir, 'escaped');
    fs.mkdirSync(root);
    fs.mkdirSync(escaped);
    ...
    expect(() => assertWithin('link-out/page.svelte', root)).toThrow(PathSafetyError);
