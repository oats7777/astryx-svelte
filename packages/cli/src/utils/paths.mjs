 ...
 *
 * Finds packages/svelte, project root, and CLI package root.
 */
...
/**
 * Find packages/svelte directory by walking up from startDir.
 * Also checks node_modules/@astryxdesign/svelte for installed usage.
 */
export function findSvelteDir(startDir = process.cwd()) {
  let dir = path.resolve(startDir);

  for (let i = 0; i < 5; i++) {
    const candidate = path.join(dir, 'packages', 'svelte');
    if (fs.existsSync(candidate)) {
      return candidate;
    }

    const nodeModules = path.join(dir, 'node_modules', '@astryxdesign', 'svelte');
  ...
  const scanDir = (searchDir) => {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (pkg.name === '@astryxdesign/svelte') continue;
