 ...
 *
 * Theme guidance must match the runtime recommendation emitted by
 * @astryxdesign/svelte/theme.
 *
...
export function getNextSteps(runPrefix) {
  return [
    '',
    '  Next steps:',
    "    1. Import base styles: import '@astryxdesign/svelte/styles.css'",
    "    2. Import components: import { Button } from '@astryxdesign/svelte/actions'",
    "    3. Add a theme: import { Theme, defineTheme } from '@astryxdesign/svelte/theme'",
    "       const appTheme = defineTheme({ name: 'app' })",
    '       <Theme theme={appTheme}>...</Theme>',
    `       For custom themes, run \`${runPrefix} astryx theme build <file>\` to emit CSS.`,
...
async function runTemplate(targetDir, {interactive = true, templateName} = {}) {
  ...
  if (!interactive) {
    ...
    const outputDir = path.resolve(targetDir, `./src/routes/${templateName}`);
    const srcPath = path.join(CLI_ROOT, 'templates', 'pages', templateName, 'page.svelte');
    fs.mkdirSync(outputDir, {recursive: true});
    fs.copyFileSync(srcPath, path.join(outputDir, 'page.svelte'));
    humanLog(`✓ Template created at ${path.relative(targetDir, outputDir)}/page.svelte`);
    return;
  }

  const TEMPLATE_OPTIONS = [
    {value: 'skip', label: 'Skip — No template'},
    ...templates.map(name => ({value: name, label: `${name} — Svelte page template`})),
  ];
  ...
  const targetPath = isCancel(
    await p.text({
      ...
      initialValue: `./src/routes/${templateChoice}`,
      placeholder: `./src/routes/${templateChoice}`,
    }),
  );

  const outputDir = path.resolve(targetDir, targetPath);
  const srcPath = path.join(CLI_ROOT, 'templates', 'pages', templateChoice, 'page.svelte');

  fs.mkdirSync(outputDir, {recursive: true});
  fs.copyFileSync(srcPath, path.join(outputDir, 'page.svelte'));

  p.log.success(`Template created at ${path.relative(targetDir, outputDir)}/page.svelte`);
}
...
export function registerInit(program) {
  program
    ...
    .action(async (options) => {
      ...
      p.note(
        'A Svelte 5 design system for building internal tools\nwith token-backed components and CLI docs.',
        'About',
      );
