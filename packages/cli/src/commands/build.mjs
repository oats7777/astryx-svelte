...
export function registerBuild(program) {
  program
    ...
    .action(async (query, options) => {
      ...
      humanLog('');
      if (directMatch) {
        humanLog(`START → Scaffold the \`${pages[0].name}\` page template, then adapt: ${run} astryx template ${pages[0].name} ./src/App.svelte`);
      ...
      humanLog('');
      humanLog('SETUP — import "@astryxdesign/svelte/styles.css" for plain CSS, or "@astryxdesign/svelte/tailwind.css" for Tailwind. Use Svelte components plus token-backed styling.');
      humanLog('');
    });
}
