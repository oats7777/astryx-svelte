...
describe('init non-interactive safety', () => {
  ...
  it('writes Svelte guidance for init --features agents --agent codex in a Svelte project', () => {
    fs.writeFileSync(
    ...
    const r = runCli(['init', '--features', 'agents', '--agent', 'codex']);

    expect(r.signal).toBeNull();
    expect(r.status).toBe(0);
    const agents = fs.readFileSync(path.join(tmpDir, 'AGENTS.md'), 'utf-8');
    expect(agents).toContain('@astryxdesign/svelte/tailwind.css');
    expect(agents).toContain('--framework svelte');
  });
});
