...
export function registerTemplate(program) {
  program
    ...
    .option('--framework <framework>', 'Template framework: svelte', 'svelte')
      ...
      switch (result.type) {
        case 'template.list': {
          ...
          const renderEntry = t => {
            ...
            const pkg =
              t.package && t.package !== '@astryxdesign/svelte'
                ? `  [${t.package}]`
                : '';
        ...
        case 'template.skeleton': {
          const {template: tName, description, components, skeleton, framework} = result.data;
          humanLog(`\n# ${tName}${description ? ' — ' + description : ''}`);
          humanLog(`# Framework: ${framework}`);
 ...
 */
async function detectTemplateCollision(name, targetPath, options = {}) {
  ...
  const match = templates.find(t => {
    if (t.dirName !== name) return false;
    if (options.package) {
      return (t.package ?? '@astryxdesign/svelte') === options.package;
    }
    return true;
  });
