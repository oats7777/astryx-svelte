...
describe('generateCompressedIndex', () => {
  ...
  it('defaults to the CSS-variable styling path (no compiler)', () => {
    const result = generateCompressedIndex('1.0.0');
    expect(result).toMatch(/class\/style with tokens/);
    expect(result).toMatch(/var\(--color-\*/);
    expect(result).toContain('@astryxdesign/svelte/styles.css');
    expect(result).not.toMatch(/Tailwind utilities backed by tokens/);
  });

  it('keeps CSS-variable guidance for unknown styling systems', () => {
  ...
  it('recommends Tailwind utilities when Tailwind is configured', () => {
    ...
    expect(result).toMatch(/@astryxdesign\/svelte\/tailwind\.css/);
  });

  it('includes Svelte framework guidance when requested', () => {
    const result = generateCompressedIndex('1.0.0', {hasSvelte: true});
    expect(result).toContain('@astryxdesign/svelte/styles.css');
    expect(result).toContain('--framework svelte');
  });

  it('includes upgrade command and migration rule', () => {
    const result = generateCompressedIndex('1.0.0');
    expect(result).toContain('upgrade --apply');
    expect(result).toMatch(/after any @astryxdesign\/svelte bump/);
  });
...
describe('detectStylingSystem', () => {
  ...
  it('ignores StyleX compiler packages in Svelte projects', () => {
  ...
  it('prefers Tailwind when both Tailwind and StyleX packages are configured', () => {
    writePkg({'@stylexjs/babel-plugin': '0.0.1', tailwindcss: '4.0.0'});
    expect(detectStylingSystem(tmpDir)).toBe('tailwind');
  });
});

describe('detectSvelteTarget', () => {
  function writePkg(deps) {
    fs.writeFileSync(
      path.join(tmpDir, 'package.json'),
      JSON.stringify({name: 'x', devDependencies: deps}),
  ...
  it('detects Svelte target packages', () => {
    writePkg({svelte: '5.0.0', '@astryxdesign/svelte': '0.0.0'});
    expect(detectSvelteTarget(tmpDir)).toBe(true);
  });

  it('returns false without Svelte signals', () => {
    writePkg({react: '19.0.0'});
    expect(detectSvelteTarget(tmpDir)).toBe(false);
  });
});
...
describe('installAgentDocs', () => {
  function setupSveltePackage(dir, version = '1.0.0') {
    const svelteDir = path.join(dir, 'node_modules', '@astryxdesign', 'svelte');
  ...
  it('onlyReplace: updates files that have XDS markers', () => {
    setupSveltePackage(tmpDir, '2.0.0');
    fs.writeFileSync(
      path.join(tmpDir, 'CLAUDE.md'),
