...
describe('import hint correctness', () => {
  ...
  describe('CLI --framework svelte shows Svelte import paths', () => {
    it('npx astryx component Button --framework svelte --detail brief uses @astryxdesign/svelte', () => {
      const result = runCli([
        'component',
        'Button',
        '--framework',
      ...
      expect(result.code).toBe(0);
      expect(result.stdout).toContain('@astryxdesign/svelte/actions');
      expect(result.stdout).not.toContain('@astryxdesign/core/Button');
    });

    it('npx astryx component --framework svelte --list hides internal probes', () => {
      const result = runCli(['component', '--framework', 'svelte', '--list']);
      expect(result.code).toBe(0);
      expect(result.stdout).not.toContain('PackageTopologyProbe');
    });

    it('npx astryx swizzle --framework svelte --list hides internal probes', () => {
      const result = runCli(['swizzle', '--framework', 'svelte', '--list']);
      expect(result.code).toBe(0);
      expect(result.stdout).not.toContain('PackageTopologyProbe');
    });

    it('npx astryx component --framework svelte --category Actions --detail compact uses bare names', () => {
      const result = runCli([
        'component',
        '--framework',
        'svelte',
      ...
      expect(result.code).toBe(0);
      expect(result.stdout).toContain('  Button  \u2190 @astryxdesign/svelte/actions');
      expect(result.stdout).toContain('  ButtonContent  \u2190 @astryxdesign/svelte/actions');
      expect(result.stdout).not.toContain('XDSButton');
      expect(result.stdout).not.toContain('XDSButtonContent');
    });
  });
