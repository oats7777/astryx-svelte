...
describe('doctor — Svelte project checks', () => {
  ...
  it('treats plain CSS projects without Tailwind as supported', async () => {
    writePackageJson({
      dependencies: {
        '@astryxdesign/svelte': '0.0.0',
        '@astryxdesign/tokens': '0.0.0',
    ...
    expect(find(report.checks, 'styles-import')?.status).toBe('pass');
    expect(find(report.checks, 'tailwind')?.status).toBe('info');
    expect(find(report.checks, 'tailwind')?.message).toContain('plain CSS');
  });

  it('warns when the Tailwind entrypoint is imported without Tailwind declared', async () => {
    writePackageJson({
      dependencies: {
        '@astryxdesign/svelte': '0.0.0',
        '@astryxdesign/tokens': '0.0.0',
    ...
    expect(find(report.checks, 'styles-import')?.status).toBe('pass');
    expect(find(report.checks, 'tailwind')?.status).toBe('warn');
    expect(find(report.checks, 'tailwind')?.fix).toContain('@astryxdesign/svelte/styles.css');
  });
