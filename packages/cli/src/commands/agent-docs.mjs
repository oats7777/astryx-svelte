...
// allow: SIZE_OK — agent-doc generation keeps one ordered prompt contract; Svelte additions are conditional lines in that contract.
...
import {findSvelteDir, CLI_ROOT} from '../utils/paths.mjs';
 ...
 *
 * Tailwind utilities need Tailwind. Plain CSS variables always work, so
 * they're the safe default.
 *
 * @param {string} targetDir
...
export function detectStylingSystem(targetDir) {
  try {
    ...
    if ('tailwindcss' in deps || '@tailwindcss/vite' in deps || '@tailwindcss/postcss' in deps) return 'tailwind';
    return 'css';
  } catch {
...
/**
 * Detect whether the consumer project is using the Svelte Astryx package.
 * @param {string} targetDir
 * @returns {boolean}
 */
export function detectSvelteTarget(targetDir) {
  try {
    const pkgPath = path.join(targetDir, 'package.json');
    if (!fs.existsSync(pkgPath)) return false;
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf-8'));
    const deps = {...pkg.dependencies, ...pkg.devDependencies};
    return 'svelte' in deps || '@astryxdesign/svelte' in deps;
  } catch {
    return false;
  }
}
 ...
 */
export function generateCompressedIndex(version, {svelteDir, runPrefix = getRunPrefix(), stylingSystem = 'css'} = {}) {
  ...
  if (svelteDir) {
    try {
      const comps = discoverSvelteComponents(svelteDir);
      let total = 0;
  ...
  lines.push('SETUP (once, in your Svelte app entry) — without this, components render unstyled:');
  lines.push(stylingSystem === 'tailwind' ? '  import "@astryxdesign/svelte/tailwind.css";' : '  import "@astryxdesign/svelte/styles.css";');
  lines.push('');
  ...
  lines.push('3. `astryx component <Name> --framework svelte` — props + examples for every component you use.');
  lines.push('   Components: `astryx component <Name> --framework svelte`; templates: `astryx template app-shell --framework svelte`.');
  lines.push('');
  ...
  if (stylingSystem === 'tailwind') {
    lines.push('- Custom styling: component props first; else Tailwind utilities backed by tokens (bg-surface, text-primary, rounded-lg) via @astryxdesign/svelte/tailwind.css. No raw hex/px.');
  } else {
    lines.push('- Custom styling: component props first; else class/style with tokens — var(--color-*|--spacing-*|--radius-*). No raw hex/px.');
  }
  ...
  lines.push('  component <Name>   inspect Svelte components');
  lines.push('  template app-shell inspect Svelte templates');
  lines.push('  upgrade --apply    run after any @astryxdesign/svelte bump');
 ...
 */
export function getXdsVersion(packageDir) {
  if (packageDir) {
...
export function injectAgentsMd(targetDir, version) {
  ...
  const compressedIndex = generateCompressedIndex(version, {
    svelteDir: findSvelteDir(targetDir),
...
export function installAgentDocs(targetDir, {zh = false, lang, agent, paths, onlyReplace = false} = {}) {
  const svelteDir = findSvelteDir(targetDir);
  const version = getXdsVersion(svelteDir);
