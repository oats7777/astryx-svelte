...
import {findShowcase} from '../api/template.mjs';
...
describe('component() sub-component scoping', () => {
  it('component("Button") can resolve Svelte docs and import hints', async () => {
    const result = await component('Button', {...CWD, framework: 'svelte', dense: true});

    expect(result.type).toBe('component.detail');
    expect(result.data.name).toBe('Button');
    expect(result.data.framework).toBe('svelte');
    expect(result.data.import).toBe('@astryxdesign/svelte/actions');
    expect(result.data.usage.description).toMatch(/Svelte/i);
  });

  it('component("Button") can return Svelte source', async () => {
    ...
    expect(result.data.component).toBe('Button');
    expect(result.data.framework).toBe('svelte');
    expect(result.data.source).toContain('<script');
  });

  it('component() rejects Svelte source traversal names', async () => {
    await expect(
      component('../../../../apps/example-svelte-tailwind/src/App', {
        ...CWD,
        framework: 'svelte',
        source: true,
      }),
