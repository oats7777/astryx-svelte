...
export class Project {
  ...
  async components() {
    return this.#memo('components', async () => {
      const svelteDir = findSvelteDir(this.#cwd);
      /** @type {Array<object>} */
      const records = [];
      ...
      if (svelteDir) {
        try {
          ...
          for (const [group, names] of Object.entries(grouped)) {
            const isUngrouped = names.length === 1 && names[0] === group;
            for (const name of names) {
              records.push({
                name,
                package: CORE_PACKAGE,
                group: isUngrouped ? null : group,
                docPath: findSvelteComponentDoc(svelteDir, name),
                sourcePath: findSvelteComponentSource(svelteDir, name),
                issuesUrl: undefined,
              });
            }
          }
        } catch {
          // Svelte discovery failure is surfaced by doctor and component commands.
        }
      }
