...
class Emitter {
  ...
  emitSlotValue(slot) {
    ...
    if (value == null) {
      return [`${INDENT.repeat(0)}<!-- TODO(xle): @${slot.key} -->`];
    }
    ...
    if (value.hint) {
      return this.emitHint(value.hint, 1).map(l => l.trimStart()).map(l => INDENT + l);
    }
    if (value.idref) {
   ...
   * template block co-defined once) and return the Svelte component name to use.
   */
...
export function expand(doc, registry, options = {}) {
  ...
  const stateLines = emitter.states.flatMap(s => [
    `let ${s.name} = ${s.initial};`,
    `function ${s.setter}(next: typeof ${s.name}) { ${s.name} = next; }`,
  ]);

  const scriptLines = [...importLines, ...stateLines];
  ...
  const code = [
    `<!-- Generated by \`astryx layout expand\` - edit freely. -->`,
    '<script lang="ts">',
