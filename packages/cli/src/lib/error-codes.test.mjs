...
const ERROR_CODES_SOURCE = path.resolve(__dirname, './error-codes.mjs');
const ERROR_CODES_DTS = path.resolve(__dirname, '../types/error-codes.d.ts');
...
function declaredErrorCodes() {
  const source = fs.readFileSync(ERROR_CODES_DTS, 'utf-8');
  const match = /export type ErrorCode =([\s\S]*?);/.exec(source);
  expect(match).not.toBeNull();
  return [...match[1].matchAll(/'([^']+)'/g)].map(([, code]) => code).sort();
}

function jsdocErrorCodes() {
  const source = fs.readFileSync(ERROR_CODES_SOURCE, 'utf-8');
  const match = /@typedef\s*{\(\s*([\s\S]*?)\s*\)}\s+ErrorCode/.exec(source);
  expect(match).not.toBeNull();
  return [...match[1].matchAll(/'([^']+)'/g)].map(([, code]) => code).sort();
}
...
describe('error-codes taxonomy', () => {
  ...
  it('public ErrorCode declaration matches the runtime taxonomy', () => {
    const runtimeCodes = Object.keys(ERROR_CODES).sort();
    const helperCodes = allErrorCodes();
    const publicCodes = declaredErrorCodes();
    const docCodes = jsdocErrorCodes();

    expect(publicCodes).toEqual(runtimeCodes);
    expect(publicCodes).toEqual(helperCodes);
    expect(docCodes).toEqual(runtimeCodes);
  });
});
