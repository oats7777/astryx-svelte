 ...
 *
 * Canonical framework-neutral token defaults.
 * - *Defaults: Plain objects with default values used by emitters and themes
 *
 * SYNC: When modified, run `node scripts/generate-token-docs.mjs` to update docs.
 * CI checks for drift via --check flag.
 *
 * Domain tokens for syntax highlighting and data visualization are consumed by
 * Svelte package surfaces and generated token artifacts.
 */

// allow: SIZE_OK — canonical token data table must stay in one source of truth for generated artifacts.
