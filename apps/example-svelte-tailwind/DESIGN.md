...
Use Astryx semantic CSS variables for all color, spacing, radius, typography, and shadow values. Tailwind utilities should resolve through `@astryxdesign/svelte/tailwind.css`, so classes such as `bg-body`, `text-primary`, `rounded-lg`, and `shadow-sm` remain theme-aware.
