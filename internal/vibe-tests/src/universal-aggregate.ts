...
import {getResultsDir, writeJson, ensureCodeFiles} from './utils.js';
...
async function main() {
  ...
  // Load target source files (extract from JSON results if needed)
  ensureCodeFiles(codeDir, extension);
  const files = fs.readdirSync(codeDir).filter(f => f.endsWith(extension));

  if (files.length === 0) {
    console.error(`No ${extension} result files found`);
    process.exit(1);
  }
