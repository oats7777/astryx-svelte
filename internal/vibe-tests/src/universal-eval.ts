...
function analyzeCorrectness(
  ...
  if (tscResult) {
    ...
    for (const f of findings) {
      ...
      const alreadyPenalizedByTypeCheck = tsc.findings.some(
        typeFinding =>
          typeFinding.line !== undefined &&
          f.line !== undefined &&
          typeFinding.line === f.line,
      );
      if (!alreadyPenalizedByTypeCheck) {
...
function countStylingDecisions(code: string, target: string): number {
  ...
  if (
    ...
    const svelteClasses = code.match(/\bclass\s*=\s*["']([^"']+)["']/g) || [];
    for (const classAttr of svelteClasses) {
  ...
  for (const line of lines) {
    ...
    if (
      (target === 'baseline' ||
        target === 'astryx-tailwind' ||
        target === 'astryx-svelte') &&
      (stripped.includes('className=') || stripped.includes('class='))
    ) {
      stylingLines++;
      continue;
    }
