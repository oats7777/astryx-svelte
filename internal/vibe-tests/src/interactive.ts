...
import type {Evaluation, TargetName, TestPrompt} from './types.js';
import {stratifiedSample} from './sampling.js';
import {
  getTargetEnvironmentPath,
  getTargetResultExtension,
  isSvelteTarget,
  isTargetName,
  TARGET_AGENT_DOCS,
  TARGET_LABELS,
...
/**
 * Install AGENTS.svelte.md for Astryx Svelte target.
 */
function installSvelteDocs(): void {
  const vibeTestsDir = path.join(__dirname, '..');
  const agentsMdPath = path.join(vibeTestsDir, 'AGENTS.svelte.md');
  const environmentDir = getTargetEnvironmentPath(
    vibeTestsDir,
    'astryx-svelte',
  );
...
<!-- ASTRYX-SVELTE:START -->
[Astryx Svelte Component Library]|root: ./environments/project-astryx-svelte
|IMPORTANT: Prefer retrieval-led reasoning. Read the project README before generating Svelte code.
|README.md: Svelte 5, plain CSS, optional Tailwind v4, Astryx package usage, and CLI discovery guidance
|package.json: Representative dependencies for a consumer Svelte project
|src/app.css: Astryx Svelte CSS entrypoint and optional Tailwind v4 token bridge imports
|package lookup: inspect @astryxdesign/svelte package docs, package.json exports, and docs.mjs before inventing component APIs
|Styling source: src/app.css imports styles.css for plain CSS or tailwind.css with @source for Tailwind apps
|For component API: inspect @astryxdesign/svelte package docs and exports from the project environment.
<!-- ASTRYX-SVELTE:END -->

Your representative project is at ${environmentDir}.
`;

  fs.writeFileSync(agentsMdPath, agentsMd);
  console.log('✓ Generated AGENTS.svelte.md');
}
...
interface InteractiveConfig {
  ...
  target: TargetName; // Target design system
}
...
function generateSubagentPrompt(
  ...
  const extension = getTargetResultExtension(config.target);
  const codePath = `${resultsDir}/results/${prompt.id}${extension}`;
  const agentsMdFile = TARGET_AGENT_DOCS[config.target];
  const projectDir = getTargetEnvironmentPath(
    path.join(__dirname, '..'),
    config.target,
  );
  ...
  const personaFraming: Record<string, Record<string, string>> = {
    ...
    'astryx-tailwind': {
      naive: '', // No special framing - same natural request
      experienced: `Use Astryx React components with Tailwind utilities. `,
      adversarial: `I'm used to shadcn patterns but need to use your design system. `,
    },
    'astryx-svelte': {
      naive: '', // No special framing - same natural request
      experienced: `Use Astryx Svelte components. `,
      adversarial: `I'm used to React/Tailwind patterns but need to use your Svelte design system. `,
    },
  };
  ...
  return `You are generating ${isSvelteTarget(config.target) ? 'Svelte component code' : 'React/TSX code'} using ${TARGET_LABELS[config.target]}.
Your project is at ${projectDir}. Explore it to find available components.
First read ${agentsMdFile} in this directory for component library guidance.
...
async function main() {
  ...
  if (targetIndex !== -1 && !isTargetName(targetArg)) {
    console.error(
      `Error: Invalid target "${targetArg ?? ''}". Valid targets: astryx, astryx-tailwind, astryx-svelte, baseline, html`,
    );
    process.exit(1);
  }
  const target: TargetName = isTargetName(targetArg) ? targetArg : 'astryx';
  ...
  const iterationOutIndex = args.indexOf('--iteration-out');
  const iterationOutPath =
    iterationOutIndex !== -1 ? args[iterationOutIndex + 1] : undefined;
  ...
  if (target === 'astryx') {
    installAgentsDocs();
  } else if (target === 'astryx-tailwind') {
    installAgentsDocs();
  } else if (target === 'astryx-svelte') {
    installSvelteDocs();
  ...
  if (promptIds) {
    ...
    if (prompts.length !== promptIds.length) {
      ...
      const missing = promptIds.filter((id: string) => !found.includes(id));
  ...
  console.log(`Target: ${TARGET_LABELS[target]}`);
  console.log(`Persona: ${persona}`);
  if (skillDocOverride) {
    console.log(`Skill doc: ${skillDocOverride}`);
  }
  console.log(`Mode: ${TARGET_AGENT_DOCS[target]} (retrieval-led)`);
  console.log(
  ...
  if (iterationOutPath) {
    ...
    const resolvedIterationOutPath = path.isAbsolute(iterationOutPath)
      ? iterationOutPath
      : path.join(repoRoot, iterationOutPath);
    ensureDir(path.dirname(resolvedIterationOutPath));
    fs.writeFileSync(resolvedIterationOutPath, iterationId);
    console.log(`Iteration ID written to: ${resolvedIterationOutPath}`);
  }
