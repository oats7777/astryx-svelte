...
import {getTargetResultExtension, isSvelteTarget} from './target-config.js';
...
function createEntryFile(
  ...
  const entryPath = path.join(
    tmpDir,
    isSvelteTarget(target) ? 'entry.ts' : 'entry.tsx',
  );

  if (isSvelteTarget(target)) {
    const cssPath = path.join(tmpDir, 'preview.css');
    fs.writeFileSync(
      cssPath,
      `@import "tailwindcss";
@import "${path
        .resolve(REPO_ROOT, 'packages/tokens/dist/default.css')
        .replace(/\\/g, '/')}" layer(astryx-base);
@import "${path
        .resolve(REPO_ROOT, 'packages/tokens/dist/tailwind-theme.css')
        .replace(/\\/g, '/')}";
@import "${path
        .resolve(REPO_ROOT, 'packages/svelte/src/lib/styles.css')
        .replace(/\\/g, '/')}";

@source "${tmpDir.replace(/\\/g, '/')}/**/*.{svelte,ts}";
@source "${path
        .resolve(REPO_ROOT, 'packages/svelte/src/lib/**/*.{svelte,ts,js}')
        .replace(/\\/g, '/')}";
...
if (!(target instanceof HTMLElement)) {
  throw new Error('Missing #root preview target.');
}
...
function createIndexHtml(
  ...
  const entryFile = isSvelteTarget(target) ? 'entry.ts' : 'entry.tsx';
...
function createViteConfig(tmpDir: string, target: string): string {
  ...
  const plugins = isSvelteTarget(target)
    ? `
    svelte(),
    tailwindcss(),
    viteSingleFile(),`
      ...
      ? `
    {
      ...
      transform(code, id) {
        ...
        // Pattern: stylex.createTheme(vars, someRaw with a generated type cast)
  ...
  const imports = isSvelteTarget(target)
...
import {viteSingleFile} from 'vite-plugin-singlefile';`;
  ...
  const aliases = isSvelteTarget(target)
    ? `
    alias: {
      '@astryxdesign/svelte': '${path
        .resolve(REPO_ROOT, 'packages/svelte/src/lib')
        .replace(/\\/g, '/')}',
    },`
    : target === 'astryx' || target === 'astryx-tailwind'
      ? xdsAliases
...
function runSvelteBuildCheck(
  ...
  const tmpOut = path.join(
    VIBE_DIR,
    `.svelte-source-check-${crypto.randomUUID()}.html`,
  );
  const buildSuccess = buildPreview(filePath, target, promptId, tmpOut);
  ...
  return buildSuccess
    ...
    : {
        ...
        errors: [
          {
            line: 1,
            message: 'Svelte source failed the preview build check.',
            code: 'SVELTE_BUILD',
          },
        ],
        errorCount: 1,
...
function runTscChecks(
  ...
  for (const file of files) {
    ...
    const result = isSvelteTarget(target)
      ? runSvelteBuildCheck(filePath, target, promptId)
      : runTscCheck(filePath, target);
  ...
  const checkerName = isSvelteTarget(target) ? 'svelte-vite' : 'tsc';
  console.log(
    `  ✓ ${checkerName}: ${cleanCount}/${Object.keys(buildErrors).length} clean, ${totalErrors} total errors`,
  );
...
async function main() {
  ...
  for (const iterationId of iterations) {
    ...
    const iterManifest = readJson<{config?: {target?: TargetName}}>(
      manifestPath,
    );
    ...
    // Extract source from JSON results if no target source files exist yet
    ensureCodeFiles(codeDir, extension);

    const files = fs.readdirSync(codeDir).filter(f => f.endsWith(extension));

    // Run tsc type checking on raw generated files BEFORE auto-import fixes
    runTscChecks(codeDir, iterDir, target, files, extension, prompts);
  ...
  if (
    !tscOnly &&
    (failedPreviews > 0 || (attemptedPreviews > 0 && totalPreviews === 0))
  ) {
    console.error(
      `\n✗ Preview build failed: ${failedPreviews}/${attemptedPreviews} preview(s) failed`,
    );
    process.exit(1);
  }
}
